...
class DescriptionCest
{
    ...
    function _before(FunctionalTester $I) { // so as _beforeClass
        ...
        $this->file = __DIR__.'/../../../../rules/'.$domain_name.'/meta_description.json.cached';
    ...
    function checkDescription(\Step\Functional\Common\Steps $I)
    {

        for ($j = 0; $j < count(self::$json['data']); $j++) {
            $I->sendRequestAndCheckDescription(self::$url . self::$json['data'][$j]['k'], self::$json['data'][$j]['v']);
        }
    }
