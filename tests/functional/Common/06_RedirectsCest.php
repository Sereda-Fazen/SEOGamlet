...
class RedirectsCest
{
    ...
    function _before(FunctionalTester $I) { // so as _beforeClass
        ...
        $this->file = __DIR__.'/../../../../rules/'.$domain_name.'/redirects.json.cached';
    ...
    function checkRedirects(\Step\Functional\Common\Steps $I)
    {
        for ($j = 0; $j < count(self::$json['data']); $j++) {
            $I->sendRequestAndCheckRedirects(self::$url . self::$json['data'][$j]['k'], self::$json['data'][$j]['v']);
        }
    }
