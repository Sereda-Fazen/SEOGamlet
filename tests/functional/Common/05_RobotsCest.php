...
class RobotsCest
{
    ...
    function _before(FunctionalTester $I) { // so as _beforeClass
        ...
        $this->file = __DIR__.'/../../../../rules/'.$domain_name.'/robots.json.cached';
    ...
    function checkRobots(\Step\Functional\Common\Steps $I)
    {
        for ($j = 0; $j < count(self::$json['data']); $j++) {
            $I->sendRequestAndCheckRobots(self::$url . self::$json['data'][$j]['k'], self::$json['data'][$j]['v']);
        }
    }
